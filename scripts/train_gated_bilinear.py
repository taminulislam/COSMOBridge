"""Train PointCloud model with Gated Bilinear HyperNetwork fusion.

Novel fusion: HyperNetwork generates T-dependent weights for bilinear
surface×graph interaction. ~10K fusion params vs 863K cross-attention.

Trains two versions:
A. GBH PointCloud: original data (152 samples)
B. GBH MoE: merged_v5 with balanced sampling
"""
...
from src.models.fusion.gated_bilinear_hyper import GatedBilinearHyperFusion
...
class GBHPointCloudModel(nn.Module):
    """PointCloud model with Gated Bilinear HyperNetwork fusion."""

    def __init__(self, config=None, feature_dim=25, pretrained_gnn_path=None):
        super().__init__()
        dropout = 0.3
        ...
        self.fusion = GatedBilinearHyperFusion(
            pointcloud_dim=256, graph_dim=256, tabular_dim=feature_dim,
            fused_dim=256, thermo_dim=5, hyper_hidden=32, dropout=dropout)
...
class GBHMoE(nn.Module):
    """MoE with Gated Bilinear HyperNetwork fusion."""
...
def main():
    ...
    # ══════════════════════════════════════════════════════════
    # MODEL A: GBH PointCloud (original data)
    # ══════════════════════════════════════════════════════════
    print(f"\n{'='*60}")
    print("MODEL A: Gated Bilinear HyperNetwork PointCloud")
    print(f"{'='*60}")
    ...
    model_a = GBHPointCloudModel(feature_dim=len(FEATURE_COLUMNS), pretrained_gnn_path=gnn_path)
    model_a.to(device)
    ...
    print(f"  Fusion params: {n_fusion:,} (vs 863K cross-attention, 33K FiLM)")
    print(f"  Params/sample: {n_a/152:.0f} (vs Chemprop ~2K)")

    ckpt_a = Path("checkpoints/gbh_pointcloud"); ckpt_a.mkdir(parents=True, exist_ok=True)
    ...
    print(f"\n{format_metrics(metrics_a, 'GBH PointCloud')}")

    # ══════════════════════════════════════════════════════════
    # MODEL B: GBH MoE (merged_v5 + balanced sampling)
    # ══════════════════════════════════════════════════════════
    print(f"\n{'='*60}")
    print("MODEL B: Gated Bilinear HyperNetwork MoE")
    print(f"{'='*60}")
    ...
    model_b = GBHMoE(feature_dim=len(merged_features), pretrained_gnn_path=gnn_path)
    model_b.to(device)
    n_b = sum(p.numel() for p in model_b.parameters())
    print(f"  Total params: {n_b:,}")

    ckpt_b = Path("checkpoints/gbh_moe"); ckpt_b.mkdir(parents=True, exist_ok=True)
    ...
    print(f"\n{format_metrics(metrics_b, 'GBH MoE')}")
    ...
    header += " {:>12s} {:>12s}".format("GBH PC", "GBH MoE")
    print(header)
    print("  " + "-" * len(header))
    ...
    print(f"\n  Fusion parameter comparison:")
    print(f"    Cross-attention:  863,491 params")
    print(f"    FiLM:             ~33,000 params (effective)")
    print(f"    GBH (ours):       {n_fusion:,} params")
    print(f"    Concatenation:    0 params")
    ...
    with open("results/gbh_results.json", "w") as f:
        json.dump(results, f, indent=2)
    print(f"\nSaved: results/gbh_results.json")
