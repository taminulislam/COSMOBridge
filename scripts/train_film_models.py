"""PointCloud and MoE models with FiLM fusion replacing cross-attention.

Drop-in replacement: same encoders (PointNet + GAT-GNN), same prediction heads,
only the fusion mechanism changes from cross-attention (863K) to FiLM (~33K).

Trains:
A. FiLM PointCloud: on original 152 samples
B. FiLM MoE: on merged_v5 with balanced sampling
"""
...
class FiLMPointCloudModel(nn.Module):
    """Original PointCloud model with FiLM fusion instead of cross-attention.

    Same encoders, same prediction head, only fusion changes.
    """
    ...
    def __init__(self, config=None, pretrained_gnn_path=None):
        ...
        # FiLM fusion (replaces cross-attention)
        tabular_dim = len(FEATURE_COLUMNS)
        self.fusion = FiLMFusion(
            pointcloud_dim=256, graph_dim=256, tabular_dim=tabular_dim,
            fused_dim=256, dropout=dropout)

        # Same prediction head as original
...
class FiLMMoE(nn.Module):
    """MoE model with FiLM fusion instead of cross-attention."""
...
def main():
    ...
    # ══════════════════════════════════════════════════════════
    # MODEL A: FiLM PointCloud (original data)
    # ══════════════════════════════════════════════════════════
    print(f"\n{'='*60}")
    print("MODEL A: FiLM PointCloud (same as original, FiLM fusion)")
    print(f"{'='*60}")
    ...
    model_a = FiLMPointCloudModel(config=config, pretrained_gnn_path=gnn_path)
    model_a.to(device)
    n_a = sum(p.numel() for p in model_a.parameters())
    print(f"  Total params: {n_a:,}")

    # Count fusion params specifically
    n_fusion = sum(p.numel() for p in model_a.fusion.parameters())
    print(f"  FiLM fusion params: {n_fusion:,} (vs 863K for cross-attention)")

    ckpt_a = Path("checkpoints/film_pointcloud"); ckpt_a.mkdir(parents=True, exist_ok=True)
    ...
    print(f"\n{format_metrics(metrics_a, 'FiLM PointCloud')}")

    # ══════════════════════════════════════════════════════════
    # MODEL B: FiLM MoE (merged_v5 + balanced sampling)
    # ══════════════════════════════════════════════════════════
    print(f"\n{'='*60}")
    print("MODEL B: FiLM MoE (same as Fix6, FiLM fusion)")
    print(f"{'='*60}")
    ...
    model_b = FiLMMoE(feature_dim=len(merged_features), pretrained_gnn_path=gnn_path)
    model_b.to(device)
    n_b = sum(p.numel() for p in model_b.parameters())
    print(f"  Total params: {n_b:,}")

    ckpt_b = Path("checkpoints/film_moe"); ckpt_b.mkdir(parents=True, exist_ok=True)
    ...
    print(f"\n{format_metrics(metrics_b, 'FiLM MoE')}")

    # ══════════════════════════════════════════════════════════
    # COMPARISON
    # ══════════════════════════════════════════════════════════
    print(f"\n{'='*60}")
    print("COMPARISON: Cross-Attention vs FiLM vs Chemprop")
    print(f"{'='*60}")

    prev = {}
    for name, path, key in [
        ("Chemprop", "results/chemprop_results.json", "test_metrics"),
        ("PC (CrossAttn)", "results/pointcloud_results.json", None),
        ("MoE Fix6 (CrossAttn)", "results/moe_fix6_results.json", "metrics"),
    ]:
        try:
    ...
    header += " {:>12s} {:>12s}".format("FiLM PC", "FiLM MoE")
    print(header)
    print("  " + "-" * len(header))
    ...
    for p in TARGET_COLUMNS:
        ...
        line = "  {:<15s}".format(p)
        for name in prev:
            line += " {:16.4f}".format(prev[name].get(key, float('nan')))
        line += " {:12.4f} {:12.4f}".format(metrics_a[key], metrics_b[key])
        print(line)

    line = "  {:<15s}".format("AVERAGE")
    for name in prev:
        line += " {:16.4f}".format(prev[name].get('avg_r2', float('nan')))
    line += " {:12.4f} {:12.4f}".format(metrics_a['avg_r2'], metrics_b['avg_r2'])
    print(line)

    print(f"\n  Param comparison:")
    print(f"    Chemprop:           ~300K")
    print(f"    PC (CrossAttn):     3,717,514 (fusion: 863K)")
    print(f"    FiLM PointCloud:    {n_a:,} (fusion: {n_fusion:,})")
    print(f"    FiLM MoE:           {n_b:,}")

    # Save
    results = {
        "film_pointcloud": {
            "n_params": n_a,
            "n_fusion_params": n_fusion,
    ...
    with open("results/film_models_results.json", "w") as f:
        json.dump(results, f, indent=2)
    print(f"\nSaved: results/film_models_results.json")
