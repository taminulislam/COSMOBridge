"""MoE-A + Fix6: Aggressive gamma1 filter + original oversampling.

Builds on Fix5 (merged_v5 with aligned gamma1 distributions) and adds
WeightedRandomSampler to oversample original data during training.
This balances gradient contributions so the model learns from both sources
equally, despite original being only 4% of the dataset.
"""
...
def make_balanced_sampler(csv_path):
    """Create WeightedRandomSampler that balances original vs ILThermo.

    Each source gets equal total sampling probability, so original samples
    are seen ~(n_ilthermo/n_original) times more often per epoch.
    """
    ...
    # Weight so total probability per source is equal
    # original sample weight: 0.5 / n_orig
    # ilthermo sample weight: 0.5 / n_ilth
    weights = np.where(is_original, 0.5 / max(n_orig, 1), 0.5 / max(n_ilth, 1))
    weights = torch.from_numpy(weights).double()

    # num_samples = full dataset size so epoch length stays the same
    ...
    print(f"  Expected original samples per epoch: ~{len(df) * 0.5:.0f} "
          f"(was {n_orig} without oversampling)")
...
def main():
    ...
    # ── Load data ──
    print(f"\n{'='*60}")
    print("MoE-A + Fix6: Aggressive Filter + Balanced Sampling")
    print(f"{'='*60}")
    ...
    ckpt_dir = Path("checkpoints/moe_fix6")
    ...
    print(format_metrics(metrics, "MoE-A Fix6 (filter + balanced sampling)"))
    ...
    prev = {}
    for name, path in [("Fix3", "results/moe_fix3_results.json"),
                        ("Fix5", "results/moe_fix5_results.json"),
    ...
    for p in TARGET_COLUMNS:
        ...
        line = f"  {p:15s}"
        for name in prev:
            line += f" {prev[name].get(key, float('nan')):10.4f}"
        line += f" {metrics[key]:10.4f}"
        print(line)

    line = f"  {'AVERAGE':15s}"
    for name in prev:
        line += f" {prev[name].get('avg_r2', float('nan')):10.4f}"
    line += f" {metrics['avg_r2']:10.4f}"
    print(line)

    # ── Save ──
    results = {
        "fix": "aggressive_filter_balanced_sampling",
        "description": "merged_v5 (gamma1 mean+/-2std filter) + WeightedRandomSampler "
                       "balancing original vs ILThermo 50/50 per epoch",
        "metrics": {k: float(v) if isinstance(v, (float, np.floating)) else v
                    for k, v in metrics.items()},
        "gate_weights": gate_weights.mean(axis=0).tolist(),
    }
    with open("results/moe_fix6_results.json", "w") as f:
        json.dump(results, f, indent=2)
    print(f"\nSaved: results/moe_fix6_results.json")
